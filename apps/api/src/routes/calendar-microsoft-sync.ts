  ...
  app.post('/v0/calendar/microsoft/disconnect', async (context) => {
    return withDatabase(context, async (db) => {
      ...
      emitAuditEvent({
        ...
        route: '/v0/calendar/microsoft/disconnect',
        statusCode: 200,
        disconnected,
      });

      return context.json({ ok: true, provider: MICROSOFT_CALENDAR_PROVIDER, disconnected });
    });
  });
      ...
      try {
        ...
        await db.transaction(async (transaction) => {
          ...
          await consumeDemoFeatureCredits(transaction as DemoQuotaDb, context.env, authedUser, {
            ...
            metadata: { provider: MICROSOFT_CALENDAR_PROVIDER, busyWindowCount: dedupedBusyWindows.length },
            now,
          });
        });
        ...
        emitAuditEvent({
          ...
          actorUserId: authedUser.id,
          provider: MICROSOFT_CALENDAR_PROVIDER,
          route: '/v0/calendar/microsoft/sync',
          statusCode: 200,
