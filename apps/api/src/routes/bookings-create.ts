import { bookingCreateSchema } from '@opencalendly/shared';
...
export const registerBookingCreateRoutes = (app: ApiApp): void => {
  app.post('/v0/bookings', async (context) => {
    ...
    return withDatabase(context, async (db) => {
      ...
      try {
        const result = await createOneOnOneBooking(db, context.env, authedUser, {
          username: payload.username,
          eventSlug: payload.eventSlug,
          startsAt: payload.startsAt,
          timezone,
        ...
        if (error instanceof BookingNotFoundError) {
          const responseBody = { ok: false, error: 'Event type not found.' };
          await completeIdempotencyRequest(db, {
            scope: 'booking_create',
            keyHash: idempotencyState.keyHash,
            statusCode: 404,
